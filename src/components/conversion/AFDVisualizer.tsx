...
import { generateAFDDot, downloadDOT, downloadSVG } from '../../utils/afndGraphGenerator';

interface AFDVisualizerProps {
...
const AFDVisualizer: React.FC<AFDVisualizerProps> = ({ 
  result, 
  initialState, 
  symbols,
  title = 'Diagrama del AFD' 
}) => {
  ...
  useEffect(() => {
    const code = generateAFDDot(result, initialState, symbols);
    setDotCode(code);
  ...
  }, [result, initialState, symbols, isVisible]);

  const handleDownloadDOT = () => {
    downloadDOT(dotCode, 'afd.dot');
  };
